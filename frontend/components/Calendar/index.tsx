...
const calculateDatesInMonth = () => {
  ...
  const amountOfDays = getDaysInMonth(2, 2022);

  const firstDayOfMonthLong = firstDay.toLocaleDateString("nl-NL", {
    weekday: "long",
  });

  const lastDayOfMonthLong = lastDay.toLocaleDateString("nl-NL", {
    weekday: "long",
  });

  const indexOfWeekFirstDay = dayNames.indexOf(firstDayOfMonthLong, 0) + 1;
  const indexOfWeekLastDay = dayNames.indexOf(lastDayOfMonthLong, 0) + 1;

  //console.log(dayNames.indexOf(lastDayOfMonthLong, 0) + 1);

  let dates = [];

  for (let i = 1; i <= amountOfDays; i++) {
    dates.push({
      date: `2022-02-${convertNumber(i)}`,
      isCurrentMonth: true,
      events: [],
    });
  }
  ...
  for (let i = 1; i <= indexOfWeekFirstDay - 1; i++) {
    const d = new Date();
  ...
  for (let i = 1; i <= 7 - indexOfWeekLastDay - 1; i++) {
    const d = new Date();
    d.setDate(firstDay.getDate() + i);
    dates.push({
      date: d.toISOString().split("T")[0],
...
export default function Example() {
  const [dates, setDates] = React.useState<Day[]>(calculateDatesInMonth());

  useEffect(() => {
    const refactored = dates.map((i: Day) => {
  ...
  return (
    <div className="lg:flex lg:flex-col w-full flex-grow">
      <header className="relative z-20 flex items-center justify-between border-b border-gray-200 py-4 px-6 lg:flex-none">
        ...
        <div className="flex items-center">
          ...
          <div className="hidden md:ml-4 md:flex md:items-center">
            ...
            <button
              type="button"
              className="bg-fluoGreen text-white p-2 rounded-md px-4"
            >
              Add event
            </button>
          </div>
